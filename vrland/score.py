...
# %% auto 0
__all__ = ['check_inversevvl', 'scale_metrics', 'compute_distdeg', 'bycluster_median', 'compute_vr_scores']

# %% ../nbs/00_score.ipynb 12
def check_inversevvl(dataframe, param_dict):
    """
    Computes inverse of RNA velocity vector lengths if those are being used as the dynamic metric
    (i.e. param_dict["dynamic]=="velocity_length").
    
    Parameters
    ----------
    dataframe
        Input Cell x Meta pandas dataframe
    param_dict
        Parameter dictionary with "dynamic" key set.
        
    Returns
    -------
    param_dict : dict
        Updated parameter dictionary with "inverse_vvl" set as the default dynamic metric.
    """
...
def scale_metrics(dataframe, metrics = ["CCAT","inverse_vvl"]):
    ...
    dataframe
        Input Cell x Meta pandas dataframe
    metrics
        Metrics to scale. Should be found within the dataframe's columns.
        
    Returns
    -------
    dataframe : pandas.DataFrame
        Updated Cell X Metadata dataframe with scaled metrics (in-place)
    """
...
def bycluster_median(dataframe, param_dict):
    ...
    dataframe
        Input Cell x Meta pandas dataframe
    param_dict
        Parameter dictionary with "potency", "dynamic", and "cluster" keys set.
        
    Returns
    -------
    dataframe : pandas.DataFrame
        Updated Cell X Metadata dataframe with median metrics.
    """
...
def compute_vr_scores(dataframe, param_dict, global_dist = False):
    ...
    dataframe
        Input Cell x Meta pandas dataframe
    param_dict
        Parameter dictionary with "potency" and "dynamic" keys set.
